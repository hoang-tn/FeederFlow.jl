...
function voltage_scale(network::FeederFlow.NetworkModel, node::FeederFlow.BusPhase)
    if haskey(network.buses, node.bus)
        return network.buses[node.bus].vbase / network.base.Vbase
    end
    if FeederFlow.is_source_internal_slack_bus(node.bus)
        return FeederFlow.kv_to_vbase(network.source.basekv, network.source.phases) / network.base.Vbase
    end
    error("Unknown bus in Y-bus order: $(node.bus)")
end

v_scale_full(ybus::FeederFlow.YBusModel, network::FeederFlow.NetworkModel) =
    [voltage_scale(network, node) for node in ybus.all_order]
