...
@testset "single global voltage base - IEEE37" begin
    network = parse_file(IEEE37_DSS)
    @test isapprox(network.base.Vbase, 4800.0 / sqrt(3); rtol = 1e-9)
    @test isapprox(network.buses["sourcebus"].vbase, 230_000.0 / sqrt(3); rtol = 1e-9)
    @test isapprox(network.buses["799"].vbase, 4800.0 / sqrt(3); rtol = 1e-9)
    @test isapprox(network.buses["799r"].vbase, 4800.0 / sqrt(3); rtol = 1e-9)
    @test isapprox(network.buses["775"].vbase, 480.0 / sqrt(3); rtol = 1e-9)
end
