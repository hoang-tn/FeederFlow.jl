...
function key_phase_index(key::AbstractString)
    parts = split(key, '.')
    length(parts) == 2 || return nothing
    tryparse(Int, parts[2])
end
...
@testset "Power flow correctness rigorous - IEEE240 (general)" begin
    network = FeederFlow.parse_file(IEEE240_DSS)
    ...
    allowed_missing_expected = Set(
        key for key in keys(dss_system)
        if key_phase_index(key) == 4
    )

    assert_voltage_map_keys(
        actual_system,
        dss_system;
        label = "IEEE240 system-base PU",
        allowed_missing_expected = allowed_missing_expected,
    )
