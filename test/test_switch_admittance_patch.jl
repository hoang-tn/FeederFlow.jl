...
v_scale_full(ybus::FeederFlow.YBusModel, network::FeederFlow.NetworkModel) =
    [network.buses[node.bus].vbase / network.base.Vbase for node in ybus.all_order]
    ...
    for (label, path) in benchmark_cases()
        @testset "$label batched switch toggles" begin
            ...
            for batch_names in switch_batches(names; limit = 4)
                ...
                @test result.max_diff <= SWITCH_PATCH_TOL
            end
        end
        ...
        @testset "$label cumulative switch updates" begin
            ...
            for batch_names in switch_batches(names; limit = 4)
                ...
                @test matrix_max_abs_diff(patched, rebuilt) <= SWITCH_PATCH_TOL
            end
        end
