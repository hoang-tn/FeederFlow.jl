...
@testset "Parser correctness - benchmark smoke" begin
    cases = (
        ("IEEE13", IEEE13_DSS, true),
        ("IEEE37", IEEE37_DSS, true),
        ("IEEE123", IEEE123_DSS, true),
        ("IEEE240", IEEE240_DSS, true),
        ("IEEE906", IEEE906_DSS, false),
    )

    for (network_name, dss_path, expect_regulators) in cases
        @testset "$network_name" begin
            network = parse_file(dss_path; regulator_model = :nonideal)
            @test !isempty(network.buses)
            @test !isempty(network.lines)
            @test !isempty(network.linecodes)
            if expect_regulators
                @test !isempty(network.regulators)
            else
                @test isempty(network.regulators)
            end
            @test haskey(network.buses, network.slack_bus)
            @test length(network.buses) >= length(network.loads)
        end
    end
end
