...
function prepare_data(path)
    ...
    # Keep same voltage shaping as prior near-feasible run.
    ...
    # Explicit branch current and angle-difference constraints (very loose).
    ang = deg2rad(89.0)
    for br in values(data["branch"])
        n = length(br["f_connections"])
        br["c_rating_a"] = fill(1e6, n)
    ...
    for gen in values(data["gen"])
        ...
        if startswith(name, "pv_")
            ...
            gen["cost"] = [100.0, 0.0]
            # Explicit substation constraints (very loose but finite).
            gen["pmin"] = fill(-1e3, length(gen["pg"]))
            gen["pmax"] = fill(1e3, length(gen["pg"]))
            gen["qmin"] = fill(-1e3, length(gen["qg"]))
            gen["qmax"] = fill(1e3, length(gen["qg"]))
        end
        gen["model"] = 2
        gen["ncost"] = 2
    end
